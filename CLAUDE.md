...
- `models/` — GORM 模型：User、Folder、File、FileObject、UploadTask、RecycleBinItem、ThumbnailTask。表结构自动迁移。
- `repositories/` — 数据访问层，封装数据库操作接口（GORM 实现），包括 UserRepository、FolderRepository、FileRepository、FileObjectRepository、UploadTaskRepository、RecycleBinRepository 等。
- `handlers/` — Gin 处理函数，按领域分文件（auth、file、folder、recycle_bin、user、health）。
- `middleware/` — `AuthMiddleware`（JWT Bearer token → 在 gin.Context 中设置 `user_id`）、`CORSMiddleware`、`RequestLoggerMiddleware`。
- `services/` — 业务逻辑层（依赖 repositories），包括文件服务、文件夹服务、回收站服务、缩略图服务、清理服务等。
- `logger/` — 日志模块，基于 zap 库实现结构化日志。
...
- 文件存储在磁盘 `config.storage.base_path/files/`，缩略图在 `.../thumbnails/`，临时分片在 `.../temp/`。
