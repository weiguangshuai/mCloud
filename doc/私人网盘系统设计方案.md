...
- `POST /api/auth/refresh` - 刷新访问令牌（Cookie）
- `POST /api/auth/logout` - 用户登出
- `GET /api/auth/profile` - 获取当前用户信息

**说明**：登录与刷新接口通过 `Set-Cookie` 写入 `access_token`（可选 `refresh_token`），前端不再存储 Token。
`/api/auth/logout` 负责清除相关 Cookie。
...
- 用户名密码登录
- 新用户注册
- JWT 通过 HttpOnly Cookie 保存（前端不落地存储）
...
```
1. 用户登录 → 验证用户名密码
2. 生成 JWT access_token（包含 user_id，短期有效）
3. 后端 Set-Cookie 写入 HttpOnly Cookie（access_token，可选 refresh_token）
4. 后续请求自动携带 Cookie
5. 后端中间件验证 access_token
6. access_token 过期则调用 /api/auth/refresh 刷新
...
8. **backend/middleware/auth.go** - JWT Cookie 认证中间件
...
2. **API 测试**（使用 Postman 或 curl）
   ...
   - 测试文件上传：`POST /api/files/upload`（需要登录 Cookie）
...
2. **Cookie 安全**：使用 HttpOnly + SameSite Cookie 存储 Token，公网建议开启 Secure
3. **CSRF 防护**：启用 CSRF Token 或自定义请求头校验
...
✅ **用户认证**：每个家人独立账号，JWT + HttpOnly Cookie 认证
✅ **文件组织**：支持文件夹/目录结构
✅ **照片功能**：缩略图生成 + 原图预览
