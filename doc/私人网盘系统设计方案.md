...
- **简单部署**：面向 Windows 单机 + 局域网场景，保持单体部署优先，降低家庭环境维护成本
- **职责清晰**：将认证、目录管理、文件元数据、物理存储、后台任务拆分，避免逻辑耦合
- **可恢复性**：上传任务、缩略图任务、回收站和临时文件清理都具备状态记录与重试能力
- **易扩展**：后续可以平滑增加分享链接、全文搜索、相册视图、外网访问等能力
- **易运维**：保留直接文件系统访问能力，同时通过数据库维护完整元数据与任务状态
...
│                     Gin API 单体服务（Go）                  │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ 接入层：路由、鉴权、中间件、限流、统一响应、错误处理   │  │
│  ├────────────────────────────────────────────────────────┤  │
│  │ 应用层：用户、文件夹、文件、上传、下载、预览、回收站   │  │
│  ├────────────────────────────────────────────────────────┤  │
│  │ 任务层：缩略图生成、分片合并、过期任务清理、回收站清理 │  │
│  ├────────────────────────────────────────────────────────┤  │
│  │ 基础设施层：MySQL / Redis / 本地文件系统 / 日志        │  │
│  └────────────────────────────────────────────────────────┘  │
└───────────────┬───────────────────────┬──────────────────────┘
                │                       │
       ┌────────▼────────┐     ┌────────▼────────┐
...
- 负责登录、文件浏览、上传下载、图片预览、回收站等交互
- 使用 `Pinia` 管理登录态、当前目录、上传任务和缓存列表
- 对大文件采用分片上传，对图片列表优先加载缩略图

#### 2. 接入层（Gin Router + Middleware）

- 提供 REST API、静态资源服务、健康检查接口
- 统一处理 JWT 认证、权限校验、参数校验、日志追踪和错误码映射
- 对下载接口支持 `HEAD` / `Range`，对上传接口提供幂等校验

#### 3. 应用服务层（核心业务）

- `AuthService`：注册、登录、用户资料、配额初始化
- `FolderService`：目录树维护、重命名、移动、路径重算
- `FileService`：文件列表、详情、删除、恢复、下载元数据
- `UploadService`：秒传判断、分片上传、合并、完整性校验
- `PreviewService`：缩略图读取、原图预览、图片信息提取
- `RecycleService`：软删除、恢复、过期清理、物理引用计数回收

#### 4. 任务层（异步后台任务）

- **缩略图任务**：上传完成后异步生成，避免阻塞主请求
- **上传清理任务**：定时清理过期分片、失败任务、孤儿临时文件
- **回收站清理任务**：超过保留期后执行永久删除，并同步更新 `ref_count`
- **补偿任务**：当缩略图生成失败或 Redis 丢失缓存时，可根据数据库状态重试恢复

> 建议保持“单体服务 + 内部任务调度器”模式，而不是一开始拆成多个独立微服务，更适合当前家庭私有部署场景。

### 核心模块边界

| 模块 | 主要职责 | 依赖 |
| --- | --- | --- |
| 用户与认证模块 | 用户注册、登录、JWT、配额初始化 | MySQL |
| 目录管理模块 | 根目录创建、目录树、路径维护、重名校验 | MySQL |
| 文件元数据模块 | 文件记录、物理对象引用、回收站状态 | MySQL |
| 上传模块 | 小文件直传、分片上传、秒传、断点续传 | MySQL、Redis、文件系统 |
| 下载与预览模块 | 文件流、Range、缩略图、原图预览 | MySQL、文件系统 |
| 后台任务模块 | 缩略图、清理、补偿、恢复 | MySQL、Redis、文件系统 |
| 存储适配模块 | 路径生成、文件写入、原子移动、删除 | 文件系统 |

### 关键架构决策

#### 1. 单体应用 + 内部异步任务

- 主接口服务与后台任务运行在同一 Go 进程内，部署简单、调试方便
- 使用数据库记录任务状态，避免仅依赖进程内内存队列
- 后续若任务量增大，可将任务执行器拆分为独立 worker 进程

#### 2. 逻辑文件与物理文件分离

- `files` 表描述“谁在什么目录下看到了什么文件”
- `file_objects` 表描述“磁盘上真实存在的文件对象”
- 通过 `ref_count` 支撑秒传、回收站恢复、物理文件延迟删除

#### 3. Redis 只承担高频临时状态

- Redis 用于分片上传进度、短时缓存、临时锁，不保存唯一事实数据
- 上传任务、恢复所需关键元数据必须落 MySQL
- 即使 Redis 重启，也可根据数据库快照恢复任务状态

#### 4. 文件系统保持可直访但不直接暴露权限

- 物理文件仍保存在用户指定目录，便于手动备份和系统迁移
- 对外访问仍经过应用层做鉴权，不直接暴露磁盘绝对路径
- 缩略图、临时分片、正式文件分目录管理，避免互相污染

### 关键业务链路

#### 上传链路

1. 前端计算文件基础信息并调用 `/upload/query` 或 `/upload/init`
2. 后端判断秒传命中、历史任务续传或新建上传任务
3. 分片写入临时目录，上传进度写入 Redis，任务快照落 MySQL
4. 所有分片完成后执行合并、MD5 校验、文件入库
5. 如为图片，写入缩略图任务，由后台异步生成

#### 下载 / 预览链路

1. 前端请求文件详情或列表
2. 后端基于 `files -> file_objects` 查到物理对象
3. 下载场景返回文件流并支持 `Range`
4. 预览场景优先返回缩略图，原图按需加载

#### 删除 / 恢复链路

1. 删除操作先写入 `recycle_bin`，并标记逻辑记录软删除
2. 恢复时校验原路径冲突，必要时自动重命名恢复
3. 永久删除时减少 `file_objects.ref_count`
4. 当引用计数归零后，再删除原图、缩略图和派生文件

### 技术选型

- **后端框架**：Gin（轻量、性能好，适合局域网文件服务）
- **数据库**：MySQL（保存用户、目录、文件、任务、回收站等核心元数据）
- **前端框架**：Vue 3 + Vite（构建快，适合桌面端与手机浏览器）
- **状态管理**：Pinia（管理登录态、目录树、上传任务状态）
- **UI 组件库**：Element Plus（组件成熟，表格、上传、对话框能力完整）
- **缓存系统**：Redis（用于分片上传进度、临时状态、短时缓存）
- **图片处理**：Go `image` + `imaging`（生成缩略图、提取图片尺寸）
- **认证方式**：JWT Bearer Token（当前方案落地快；若未来增强安全性，可升级为 HttpOnly Cookie）
- **日志方案**：建议使用结构化日志，便于排查上传失败、任务重试和清理任务异常

### 推荐部署形态

- **默认方案**：Gin 同时提供 API、前端静态资源和内部任务调度器
- **可选增强**：前置 Nginx 作为反向代理，处理 HTTPS、静态缓存和大文件转发
- **存储规划**：建议将正式文件、缩略图、临时分片拆分为独立子目录，便于备份与清理
...
CREATE TABLE folders (
    ...
    parent_id INT NULL,  -- NULL 表示根目

    user_id INT NOT NULL,

    is_root TINYINT(1) NULL DEFAULT NULL, -- 仅根目录，其余为NULL

    path VARCHAR(1000) NOT NULL,  -- / 开头，不以 / 结尾
...
CREATE TABLE files (
    ...
    folder_id INT NOT NULL,                -- 指向真实文件root为用户root id)
...
CREATE TABLE file_objects (
    ...
    file_md5 VARCHAR(32) NOT NULL,         -- 用于秒传与完整性验
...
- 永久删除逻辑文件ref_count -1，归零才删除物理文件

- 存储配额按逻辑文件计费（逻辑新增 +size，永久删-size
...
- 数据Redis混合方案：Redis存储高频操作的分片状态（使用Set数据结构），数据库持久化任务元数

- 支持定时清理过期任务和临时文

- uploaded_chunks字段作为Redis的备
...
CREATE TABLE recycle_bin (
    ...
    original_full_path VARCHAR(1000) COMMENT '删除前完整路用于冲突恢复判定)',

    original_folder_id INT COMMENT '删除前所在文件夹ID',

    file_object_id INT NULL COMMENT '物理文件对象ID(仅文件类',

    file_size BIGINT COMMENT '文件大小（仅文件类型,

    deleted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '删除时间',

    expires_at TIMESTAMP NOT NULL COMMENT '回收站过期时间，默认30,
...
所有时序图使用 PlantUML 格式编写，存储在 `doc/sequence-diagrams.puml` 文件中。您可以使用 PlantUML 工具或在线服务（[PlantUML Online](http://www.plantuml.com/plantuml/uml/)）来查看和渲染这些时序图
...
1. **用户注册登录流程** - 展示用户注册、登录和JWT认证的完整流

2. **文件上传流程** - 展示文件上传、缩略图生成和数据库记录创建的过

3. **文件下载流程** - 展示文件下载的权限验证和文件传输过程

4. **图片预览流程** - 展示缩略图加载和原图预览的交互流

5. **文件夹操作流* - 展示文件夹的创建、重命名和删除操
...
2. `doc/sequence-diagrams.puml` 文件内容复制粘贴到编辑器

3. 点击"Submit"按钮即可查看渲染后的时序
...
**说明**：登录接口返JWT access_token，前端存储在 localStorage 中
...
- `POST /api/files/upload/init` - ʼƬϴ񣨽ʼ/봫жϣ

- `POST /api/files/upload/chunk` - ϴƬֶ֧ϵ
...
- JWT 保存localStorage，通过 Authorization 头传递
...
redis:
  ...
  upload_task_expire: 604800      # 上传任务过期时间（7天）
...
7. 写入 files（逻辑文件记录，关file_object_id
...
1. 前端计算文件 MD5，将文件分片（每5MB

2. 先调/upload/query 接口，按文件签名查询是否有可续传任务

   - 如果命中：直接复用已upload_id + uploaded_chunks 继续上传

   - 如果未命中：再调/upload/init 初始化上传任   - /upload/init 仅负责初始化任务或秒传判断，不承担历史任务查3. 循环上传每个分片/upload/chunk
...
4. 所有分片上传完成后，调/upload/complete
   ...
   - 写入 files（逻辑文件记录，关file_object_id
...
1. 上传过程中网络中断，后端任务状态持续保存在 MySQL + Redis

2. 用户刷新页面或重新打开，前端调/upload/tasks 拉取未完成任务列3. 用户点击“继续上传”后，前端调/upload/tasks/:upload_id 获取任务详情

4. 前端优先使用 File System Access API 恢复本地文件，失败时提示重新选择同一文件

5. 校验 file_size + md5 后，跳过已上传分片，仅上传缺失分```
...
2. 生成 JWT access_token（包user_id天有效）

3. 返回 token 给前端，前端存储localStorage

4. 后续请求通过 Authorization: Bearer <token> 头携

5. 后端中间件从 Authorization 头解析并验证 token

6. token 过期需重新登录

7. Token 中提user_id，确保用户只能访问自己的文件
...
- 使用 `path` 字段存储完整路径（如：`/照片/2024/春节`），统一`/` 开头、不`/` 结尾
...
10. **backend/handlers/auth.go** - 认证处理

11. **backend/handlers/folder.go** - 文件夹处理器

12. **backend/handlers/file.go** - 文件处理
...
11. **frontend/src/views/Home.vue** - 主页
...
1. **启动后端服务**
   ...
   - 验证服务`http://localhost:8080` 启动成功

   - 验证 MySQL 数据库连接成

   - 验证数据库表自动创建（如果使用 GORM AutoMigrate

   - 验证存储目录自动创建



2. **API 测试**（使Postman curl

   - 测试用户注册：`POST /api/auth/register`

   - 测试用户登录：`POST /api/auth/login`

   - 测试文件上传：`POST /api/files/upload`（需Bearer Token
...
# 下载并安Memurai Developer Edition（免费）
...
- 对于家庭局域网使用，Redis 可以不设置密

- 如果设置了密码，需要在 config.yaml 中配

- Redis 主要用于存储分片上传的临时信息，数据会自动过期（7天）

- 如果不想安装 Redis，可以暂时跳过断点续传功能，小文件上传仍可正常使
...
database:
  ...
  username: "root"  # 或您创建mcloud 用户

  password: "your_mysql_password"  # 修改为您MySQL 密码
...
storage:
  ...
  base_path: "D:/MyCloudStorage"  # 修改为您指定的存储路
...
- 确保 Windows 防火墙允许端8080

- 查看本机 IP：在命令行运`ipconfig`，找IPv4 地址（如 192.168.1.100
...
2. 将批处理文件添加Windows 启动文件夹：

   - `Win + R`，输`shell:startup`
...
1. **JWT Secret**：修`config.yaml` 中的 JWT secret 为强密码

2. **Token 安全**：JWT 存储localStorage，注XSS 防护

3. **CORS 配置**：生产环境应Access-Control-Allow-Origin 设为具体域名

4. **密码强度**：建议在注册时添加密码强度验

5. **文件类型限制**：可以根据需要限制允许上传的文件类型

6. **文件大小限制**：默认限制为 1GB，可根据需要调
...
2. **缩略图缓*：缩略图生成后缓存，避免重复生成

3. **数据库索*：在 `user_id`、`folder_id` 等字段上建立索引

4. **静态资*：前端构建后的静态资源可以由后端直接服务



### 备份建议

1. **数据库备*：定期备SQLite 数据库文

2. **文件备份**：定期备份存储目

3. **配置备份**：备`config.yaml` 配置文件
...
**秒传功能**：基于文MD5 的秒传，相同文件复用物理文件并计

**技术栈**：Go + Gin + Vue + MySQL + Redis

**Windows 部署**：适合 Windows 本地部署，支持局域网访问

**移动端支*：响应式设计，手机浏览器可直接访问使



该方案采用前后端分离架构，代码结构清晰，易于维护和扩展。数据库使用 MySQL 存储元数据，Redis 缓存上传任务进度，两者配合实现稳定可靠的断点续传功能。文件存储采用用户隔+ 时间分类的方式，既保证了安全性，又便于直接在文件系统中管理
