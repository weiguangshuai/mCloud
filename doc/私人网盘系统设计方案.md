...
**修改说明（添加存储配额管理和软删除）**：
```sql
-- 为现有users表添加存储配额和软删除字段
ALTER TABLE users
ADD COLUMN storage_quota BIGINT DEFAULT 10737418240 COMMENT '存储配额(字节)，默认10GB',
ADD COLUMN storage_used BIGINT DEFAULT 0 COMMENT '已使用存储空间(字节)',
...
CREATE TABLE folders (
    ...
    parent_id INT DEFAULT 0,  -- 0 表示根目录
    user_id INT NOT NULL,
    path VARCHAR(1000) NOT NULL,  -- 完整路径，便于查询
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE files (
    ...
    folder_id INT DEFAULT 0,               -- 0 表示根目录
    user_id INT NOT NULL,
    ...
    width INT,                             -- 图片宽度
    height INT,                            -- 图片高度
    file_md5 VARCHAR(32),                  -- 文件MD5值，用于秒传和完整性验证
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    ...
    INDEX idx_file_md5 (file_md5)          -- 用于秒传查询
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```
...
ALTER TABLE files
ADD COLUMN deleted_at TIMESTAMP NULL DEFAULT NULL COMMENT '软删除时间',
ADD COLUMN deleted_by INT NULL COMMENT '删除操作的用户ID',
ADD INDEX idx_deleted_at (deleted_at);

-- 优化秒传安全：移除全局MD5索引，改为用户范围索引
ALTER TABLE files DROP INDEX idx_file_md5;
ALTER TABLE files ADD INDEX idx_user_md5 (user_id, file_md5) COMMENT '用户范围内的秒传查询';
```

**修复的缺陷**：
- ✅ P0-2：秒传功能安全隐患（通过用户范围索引限制秒传查询）
- ✅ P1-7：软删除和回收站

### 新增表结构

#### 4. upload_tasks（上传任务表）
```sql
...
#### 5. file_access_logs（文件访问日志表）
```sql
CREATE TABLE file_access_logs (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    file_id INT NOT NULL,
    user_id INT NOT NULL,
    action ENUM('view', 'download', 'preview', 'thumbnail') NOT NULL,
    ip_address VARCHAR(45) COMMENT 'IPv4或IPv6地址',
    user_agent VARCHAR(500),
    access_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    file_size BIGINT COMMENT '访问时的文件大小',
...
**注意**：不设置外键，避免文件删除时级联删除日志，保留历史审计记录。
...
修改说明：
1. 上传前检查用户存储配额
2. 秒传查询限制在用户范围内（WHERE user_id = ? AND file_md5 = ?）
3. 秒传成功时更新用户配额
...
**审计日志**
- `GET /api/files/:id/access-logs` - 查询文件访问日志
...
audit:
  ...
  log_actions: ["download", "preview", "view", "thumbnail"]
...
5. 后端中间件验证 Token
6. 从 Token 中提取 user_id，确保用户只能访问自己的文件
```

### 4. 文件夹路径管理
- 使用 `path` 字段存储完整路径（如：`/照片/2024/春节`）
- 便于面包屑导航和路径查询
- 创建文件夹时自动计算并存储完整路径
- 移动文件夹时需要递归更新子文件夹路径
...
6. **backend/models/file.go** - 文件模型
7. **backend/middleware/auth.go** - JWT 认证中间件
8. **backend/middleware/cors.go** - CORS 中间件
9. **backend/handlers/auth.go** - 认证处理器
10. **backend/handlers/folder.go** - 文件夹处理器
11. **backend/handlers/file.go** - 文件处理器
12. **backend/services/thumbnail_service.go** - 缩略图生成服务
13. **backend/utils/jwt.go** - JWT 工具
14. **backend/utils/password.go** - 密码加密工具
15. **backend/utils/response.go** - 统一响应格式
16. **backend/config.yaml** - 配置文件
...
2. **API 测试**（使用 Postman 或 curl）
   ...
   - 测试文件上传：`POST /api/files/upload`（需要 JWT Token）
...
2. **密码强度**：建议在注册时添加密码强度验���
3. **文件类型限制**：可以根据需要限制允许上传的文件类型
4. **文件大小限制**：默认限制为 1GB，可根据需要调整
5. **HTTPS**：如果需要外网访问，建议配置 HTTPS
